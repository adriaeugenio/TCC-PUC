...
caminhoPasta = p.pastaDados + '/Voo Regular Ativo (VRA)/'

#Verifica se o Arquivo Existe na Pasta
if os.path.exists(caminhoPasta + 'VRAGeral.csv'):
    ...
    for a in p.listaMeses:
        ...
        if(p.geraTodoHistorico == 1 and p.listaAnosGeradosBaseHistorica[indiceAnos] != p.AnoAtual) \
           ...
           or (p.geraTodoHistorico == 0 and p.listaAnosGeradosBaseHistorica[indiceAnos] == p.AnoAtual and p.listaMesesNumerico[indiceMeses] == p.MesAtual):
            ...
            nomeArquivo = "VRA_" + str(p.listaAnosGeradosBaseHistorica[indiceAnos]) + mesesNumerico  + ".csv"       

            urlMes = str(p.listaAnosGeradosBaseHistorica[indiceAnos]) + "/" + codigoMes + str(p.listaMesesNumerico[indiceMeses]) + "%20-%20" + p.listaMeses[indiceMeses] + "/" + nomeArquivo

            urlCaminho = str(p.listaAnosGeradosBaseHistorica[indiceAnos]) + "/"

            #Define a Url de Origem do Arquivo
            remote_url = p.urlOrigem + "dadosabertos/Voos%20e%20operações%20aéreas/Voo%20Regular%20Ativo%20%28VRA%29/" + urlMes

            #Define a Url o Local de Salvamento do Arquivo
            local_file = p.pastaDados + '/Voo Regular Ativo (VRA)/' + nomeArquivo
            ...
            df = pd.read_csv(local_file,sep=';',encoding='latin-1',on_bad_lines='skip', skiprows=[0])

            headerList = ['ICAO Empresa Aérea','Número Voo','Código Autorização (DI)','Código Tipo Linha','ICAO Aeródromo Origem','ICAO Aeródromo Destino','Partida Prevista','Partida Real','Chegada Prevista','Chegada Real','Situação Voo','Código Justificativa'] 
            ...
            if totalColunas == 12:
                ...
                os.rename(local_file, p.pastaDados + '/Voo Regular Ativo (VRA)/'+ 'VRA_' + str(p.listaAnosGeradosBaseHistorica[indiceAnos]) + "-" + codigoMes + mesesNumerico  + ".csv") 
...
os.chdir(p.pastaDados + '/Voo Regular Ativo (VRA)')              

if p.geraTodoHistorico == 0:  
                
    #Juntas as Fontes de Dados
    v = jp.juntaPlanilhas('Voo Regular Ativo (VRA)','VRANovo',headerList) 
    v1 = jph.juntaPlanilhasHistorico('Voo Regular Ativo (VRA)','VRAHistorico') 

else:

    #Juntas as Fontes de Dados
    v = jp.juntaPlanilhas('Voo Regular Ativo (VRA)','VRAHistorico',headerList)  

#Renomeia o arquivo
os.rename('VRAHistorico.csv', 'VRAGeral.csv')